...
sys.path.append("/home/gabriel/Documents/Ecole/2022-2023/Projet_Innov/EvData/translate_2_formats")
from events2spikes import ev2spikes

start = dt.now()

sim, options = get_simulator(("--plot-figure", "Plot the simulation results to a file", {"action": "store_true"}),
                             ("--nb-convolution", "The number of convolution layers of the model",
                              {"action": "store", "type": int}),
                             ("--grid-input", "Input size", {"action": "store", "type": int}),
                             ("--size-filter", "Input size", {"action": "store", "type": int}),
...
DIRECTIONS = {-1: "INDETERMINATE", 0: "SOUTH-EAST ↘︎", 1: "SOUTH-WEST ↙︎", 2: "NORTH-WEST ↖︎", 3: "NORTH-EAST ↗︎",
              4: "EAST →", 5: "SOUTH ↓", 6: "WEST ←",
              7: "NORTH ↑"}  # KEY=DIRECTIONS ID ; VALUE=STRING REPRESENTING THE DIRECTION
NB_DIRECTIONS = min(len(DIRECTIONS) - 1,
                    NB_CONV_LAYERS)  # No more than available directions, and at least 2 directions. -1 to ignore INDETERMINATE

### Generate input data

time_data = 50_000  # TODO de base 1e6, à réduire pour tests
...
x_input = options.grid_input
filter_x = options.size_filter
x_output = x_input - filter_x + 1

y_input = options.grid_input
filter_y = options.size_filter
...
for t in range(int(time_data / pattern_interval)):
    ...
    if direction == 0:
        start_x = randint(x_margin,
                          x_input - pattern_duration - x_margin)  # We leave a margin of 4 neurons on the edges of the input layer so that the whole movement can be seen by the convolution window
        start_y = randint(y_margin, y_input - pattern_duration - y_margin)
        input_events = np.concatenate(
            (input_events, [[start_x + d, start_y + d, 1, d + t * pattern_interval] for d in range(pattern_duration)]),
            axis=0)

    elif direction == 1:
        start_x = randint(x_input - x_margin - 1,
                          x_input - pattern_duration)  # We leave a margin of 4 neurons on the edges of the input layer so that the whole movement can be seen by the convolution window
        start_y = randint(y_margin, y_input - pattern_duration - y_margin)
        input_events = np.concatenate(
            (input_events, [[start_x - d, start_y + d, 1, d + t * pattern_interval] for d in range(pattern_duration)]),
            axis=0)
...
for in2out_conn in Input_to_output_i_conn:
    ...
    for X, Y in it.product(range(x_output), range(y_output)):
        ...
        conn = []
        # Convolution proprement dite à partir des coord
...
neuron_activity_tag = [[False for _ in range((x_input - filter_x + 1) * (y_input - filter_y + 1))] for _ in
                       range(NB_CONV_LAYERS)]
...
class visualiseTime(object):
    ...
    def recognize_movement(self, delay_matrix):
        ...
        def pred_movement(delay_matrix, rangeI, rangeJ, prevI, prevJ):
            ...
            # S'assurer que le délai min en dehors de la diagonale soit supérieur au délai du premier pixel du mouvement
            # TODO et ce, avec un écart d'au moins 0.2 ms
            if not np.amin(delay_matrix) > first_mvt_idx_val:
                return False
            return True
        ...
        if pred_movement(delay_matrix, range(1, size_matrix), range(1, size_matrix), -1, -1):
            return 0  # HGBD
        if pred_movement(delay_matrix, range(size_matrix - 2, -1, -1), range(size_matrix - 2, -1, -1), 1, 1):
            return 2  # BDHG
        if pred_movement(delay_matrix, range(1, size_matrix), range(size_matrix - 2, -1, -1), -1, 1):
            return 1  # HDBG
        if pred_movement(delay_matrix, range(size_matrix - 2, -1, -1), range(1, size_matrix), 1, -1):
            return 3  # BGHD
        ...
        if pred_movement(delay_matrix, [half_matrix] * size_matrix, range(1, size_matrix), 0, -1):
            return 4  # MGMD
        if pred_movement(delay_matrix, range(1, size_matrix), [half_matrix] * size_matrix, -1, 0):
            return 5  # MHMB
        if pred_movement(delay_matrix, [half_matrix] * size_matrix, range(size_matrix - 2, -1, -1), 0, 1):
            return 6  # MDMG
        if pred_movement(delay_matrix, range(size_matrix - 2, -1, -1), [half_matrix] * size_matrix, 1, 0):
            return 7  # MBMH
        else:
            return -1
    ...
    def print_filters(self, t):
        ...
        SCALING_VALUE = SIZE_CONV_FILTER + ((
                                                    NB_CONV_LAYERS - 2) * 4)  # Make sure the plot is big enought depending on number of convolution used
        FONTSIZE = 9 + (1.05 * NB_CONV_LAYERS)

        fig, axs = plt.subplots(nrows=len(LOG_STR), ncols=NB_CONV_LAYERS, sharex=True,
                                figsize=(SCALING_VALUE, SCALING_VALUE))
        for i in range(len(LOG_STR)):  # Delay and Weight
            for layer_n in range(NB_CONV_LAYERS):  # The number of convolution layer in the model
                ...
                if i == 0:  # Delay matrix part
                    ...
                    if direction_id != -1:  # Not INDETERMINATED ---> DETERMINATED
    ...
    def print_final_filters(self):
        ...
        imageio.mimsave(self.OUTPUT_PATH_CURRENT_RUN + '/delays_and_weights_evolution.gif', imgs,
                        duration=1)  # 1s between each frame of the gif
...
class LearningMechanisms(object):
    ...
    def __call__(self, t):
        ...
        for pre_neuron in range(self.input.size):
            if self.input_spikes._spikes[pre_neuron] != -1 and self.input_spikes._spikes[pre_neuron] > \
                    self.input_last_spiking_times[pre_neuron]:
        ...
        for post_neuron in range(self.output.size):
            if self.output_spikes._spikes[post_neuron] != -1 and self.check_activity_tags(post_neuron):
                ...
                if self.output_spikes._spikes[post_neuron] > self.output_last_spiking_times[
                    post_neuron] and not self.stop_condition(post_neuron):
                    # We actualize the last time of spike for this neuron
                    ...
                    for pre_neuron in range(len(self.DelayWeights.delay)):
                        ...
                        if not np.isnan(self.DelayWeights.delay[pre_neuron][post_neuron]) and not np.isnan(
                                self.DelayWeights.weight[pre_neuron][post_neuron]) and self.input_last_spiking_times[
                            pre_neuron] != -1 and self.output_last_spiking_times[post_neuron] != -1:
                            # Some values here have a dimension in ms
                            delta_t = self.output_last_spiking_times[post_neuron] - self.input_last_spiking_times[
                                pre_neuron] - self.DelayWeights.delay[pre_neuron][post_neuron]
    ...
    def stop_condition(self, post_neuron):
        for pre_neuron in range(len(self.DelayWeights.delay)):
            if not np.isnan(self.DelayWeights.delay[pre_neuron][post_neuron]) and self.DelayWeights.delay[pre_neuron][
                post_neuron] <= self.c:
                return True
        return False
    ...
    def actualize_filter(self, pre_neuron, post_neuron, delta_d, delta_w):
        ...
        self.filter_d[filter_coords[0]][filter_coords[1]] = max(0.01, min(
            self.filter_d[filter_coords[0]][filter_coords[1]] + delta_d, self.max_delay))
        self.filter_w[filter_coords[0]][filter_coords[1]] = max(0.05, self.filter_w[filter_coords[0]][
            filter_coords[1]] + delta_w)
        ...
        for window_x in range(0, x_input - (filter_x - 1)):
            for window_y in range(0, y_input - (filter_y - 1)):
                ...
                if not np.isnan(self.DelayWeights.delay[input_neuron_id][convo_neuron_id]) and not np.isnan(
                        self.DelayWeights.weight[input_neuron_id][convo_neuron_id]):
                    self.DelayWeights.delay[input_neuron_id][convo_neuron_id] = self.filter_d[filter_coords[0]][
                        filter_coords[1]]
                    self.DelayWeights.weight[input_neuron_id][convo_neuron_id] = self.filter_w[filter_coords[0]][
                        filter_coords[1]]
            ...
            for window_y in range(0, y_input - (filter_y - 1)):
                for x in range(len(self.filter_d)):
                    for y in range(len(self.filter_d[x])):
                        ...
                        convo_neuron_id = window_x + window_y * (x_input - filter_x + 1)
                        if input_neuron_id < self.input.size and not np.isnan(
                                self.DelayWeights.delay[input_neuron_id][convo_neuron_id]) and not np.isnan(
                            self.DelayWeights.weight[input_neuron_id][convo_neuron_id]):
...
class Metrics():
    ...
    def compute_metrics(self):
        ...
        conv_variables = {conv_id: [0, 0, 0] for conv_id in range(
            NB_CONV_LAYERS)}  # KEY=COND_ID ; VALUE=[Nb_TruePositive, Nb_FalsePositive, Nb_FalseNegative]
...
growth_factor = (
                        0.001 / pattern_interval) * pattern_duration  # <- juste faire *duration dans STDP We increase each delay by this constant each step
...
visu = visualiseTime(sampling_interval=500)  # at end of training it is used to plot result
wd_rec = WeightDelayRecorder(sampling_interval=1, proj=Input_to_Conv_i[
    0])  # permet l'enregistrement  des poids et délai à un pas de temps spécifié
...
if options.plot_figure:
    ...
    for i in range(NB_CONV_LAYERS):
        ...
        figure_params.append(Panel(Conv_i_spikes[i], xlabel="Conv" + str(i) + " spikes - " + direction_str + "(" + str(
            direction_id_of_conv) + ")", yticks=True, markersize=0.2, xlim=(0, time_data),
                                   ylim=(0, ConvLayers[i].size)))

    for i in range(NB_CONV_LAYERS):
        figure_params.append(
            Panel(Conv_i_data[i].filter(name='v')[0], xlabel="Membrane potential (mV) - Conv" + str(i) + " layer",
                  yticks=True, xlim=(0, time_data), linewidth=0.2, legend=False))

    Figure(
        # raster plot of the event inputs spike times
        Panel(Input_spikes, xlabel="Input spikes", yticks=True, markersize=0.2, xlim=(0, time_data),
              ylim=(0, Input.size)),
        *figure_params,
        title=title,
    ...
    option_string = ','.join(f"{key}={value}" for key, value in vars(options).items())
    info_string = 'placeholder'

    if len(conv_to_direction) != NB_DIRECTIONS:  # TODO replace by NB_CONV_LAYERS ?
        print(
            "Cannot compute metrics : the convolution layers did not converge towards a direction in a sufficiently large number",
            flush=True)
        info_string = ','.join(f"{k}-{v}" for k, v in conv_to_direction.items())
    else:
        ...
        info_string = ','.join(','.join(f"{metrics_names[i]}={v}"
                                        for i, v in enumerate(conv))
                               for conv in metrics.values())

    with open("Results/CSV/metrics.csv", "a+") as csv_file:
        csv_file.write(f"{option_string},{time_data=},{info_string}\n")
